...
<div
  class="w-full min-h-[calc(100vh-80px)] bg-gradient-to-br from-slate-50 via-white to-blue-50/30"
>
  <div class="max-w-[1600px] mx-auto px-6 py-10 md:px-4 md:py-6">
    <!-- Header Section with improved design -->
    <header class="mb-8 md:mb-6">
      <div
        class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium mb-3"
      >
      ...
      <p class="text-base text-slate-600 leading-relaxed max-w-2xl">
        Search aviation telegrams with full-text search and time range filters. Get instant results
        from your message database.
      </p>
    </header>
    ...
    {:else if search.results.length === 0}
      <SearchEmptyState hasSearchCriteria={search.hasSearchCriteria} onReset={search.handleReset} />
      <!-- Results Section -->
    {:else}
      <SearchResults
